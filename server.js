...
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'admin@campusnotes.local';
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'Admin@123';
const ADMIN_TOKEN = process.env.ADMIN_TOKEN || 'campusnotes-admin-token';
...
app.get('/api/notes', (req, res) => {
  ...
  const visible = notes.filter(note => note.status === 'approved' || !('status' in note));
  res.json(visible);
});
...
app.post('/api/login', (req, res) => {
  ...
  if (email === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
    return res.json({ message: 'Admin login successful', user: buildAdminUser(), token: ADMIN_TOKEN });
  }
...
app.post('/api/notes', upload.single('file'), (req, res) => {
  ...
  res.json({ message: 'Note uploaded successfully and submitted for admin review.', note });
});
...
app.get('/api/notes/:id/download', (req, res) => {
  ...
  if (note.status && note.status !== 'approved' && !isAdminRequest(req)) {
    return res.status(403).json({ error: 'Note is not approved yet.' });
  }
...
app.post('/api/admin/notes/:id/decision', (req, res) => {
  if (!isAdminRequest(req)) {
    return res.status(401).json({ error: 'Unauthorized admin access.' });
  }
  ...
  note.reviewedBy = 'admin';
  writeData(NOTES_FILE, notes);

  res.json({ message: `Note ${note.status} successfully.`, note });
});
...
app.get('/api/admin/users', (req, res) => {
  ...
  const users = readData(USERS_FILE).map(user => ({
    id: user.id,
    name: user.name,
    email: user.email,
